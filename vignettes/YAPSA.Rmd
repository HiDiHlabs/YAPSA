...
Remember that $j$ is the index over samples, $m$ is the number of samples, $i$ is the index over signatures and $l$ is the number of signatures. `LCD` uses the function `lsei` from the package `r CRANpkg("limsolve")` (https://cran.r-project.org/web/packages/limSolve/index.html) [@limsolve_package2009] and [@limsolve2009]. Note that the optimization procedure is carried out for every $V_{(\cdot j)}$, i.e. for every column of $V$ separately. Of course $W$ is constant, i.e. the same for every $V_{(\cdot j)}$.
