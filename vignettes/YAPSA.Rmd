...
Alex_signatures_df <- read.csv(Alex_signatures_path,header=TRUE,sep="\t")
kable(Alex_signatures_df[c(1:9),c(1:4)])
```

We will now reformat the data frame:

```{r reformat_old_sigs}
Alex_rownames <- paste(Alex_signatures_df[,1],Alex_signatures_df[,2],sep=" ")
select_ind <- grep("Signature",names(Alex_signatures_df))
Alex_signatures_df <- Alex_signatures_df[,select_ind]
number_of_Alex_sigs <- dim(Alex_signatures_df)[2]
names(Alex_signatures_df) <- gsub("Signature\\.","A",names(Alex_signatures_df))
rownames(Alex_signatures_df) <- Alex_rownames
kable(Alex_signatures_df[c(1:9),c(1:6)],
...
Alex_validated_signatures_df <- Alex_signatures_df[,grep("^A[0-9]+",
                                          names(Alex_signatures_df))]
number_of_Alex_validated_sigs <- dim(Alex_validated_signatures_df)[2]
```
...
Alex_COSMIC_signatures_df <- read.csv(Alex_COSMIC_signatures_path,
                                      header=TRUE,sep="\t")
Alex_COSMIC_rownames <- paste(Alex_COSMIC_signatures_df[,1],
                              Alex_COSMIC_signatures_df[,2],sep=" ")
COSMIC_select_ind <- grep("Signature",names(Alex_COSMIC_signatures_df))
Alex_COSMIC_signatures_df <- Alex_COSMIC_signatures_df[,COSMIC_select_ind]
number_of_Alex_COSMIC_sigs <- dim(Alex_COSMIC_signatures_df)[2]
names(Alex_COSMIC_signatures_df) <- gsub("Signature\\.","AC",
                                         names(Alex_COSMIC_signatures_df))
rownames(Alex_COSMIC_signatures_df) <- Alex_COSMIC_rownames
kable(Alex_COSMIC_signatures_df[c(1:9),c(1:6)],
...
Alex_COSMIC_signatures_df <- Alex_COSMIC_signatures_df[COSMIC_order_ind,]
kable(Alex_COSMIC_signatures_df[c(1:9),c(1:6)],
...
bio_process_vector <- c("spontaneous deamination","spontaneous deamination",
                        ...
                        "nonvalidated")
signatures_indices_df <- data.frame(sig=colnames(Alex_signatures_df))
signatures_indices_df$index <- seq_len(dim(signatures_indices_df)[1])
signatures_indices_df$colour <- signature_colour_vector
signatures_indices_df$process <- bio_process_vector
...
COSMIC_signatures_indices_df <- data.frame(sig=colnames(Alex_COSMIC_signatures_df))
COSMIC_signatures_indices_df$index <- seq_len(dim(COSMIC_signatures_indices_df)[1])
COSMIC_signatures_indices_df$colour <- COSMIC_signature_colour_vector
COSMIC_signatures_indices_df$process <- COSMIC_bio_process_vector
```
...
lymphoma_Nature2013_mutation_catalogue_df[c(1:9),c(5:10)]
```
\newpage
...
COSMIC_LCD_cutoff_list <- LCD_cutoff(lymphoma_Nature2013_mutation_catalogue_df,
                                     Alex_COSMIC_signatures_df,in_cutoff=my_cutoff,
                                     in_filename=NULL)
...
chosen_signatures_indices_df <- COSMIC_signatures_indices_df[lymphoma_Nature2013_COSMIC_cutoff_sig_ind,]
```
...
```{r}
kable(chosen_signatures_indices_df, row.names=FALSE,
